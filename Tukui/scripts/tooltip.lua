		...
		elseif InCombatLockdown() and db.hidecombat == true and TukuiCF["tooltip"].hidecombatraid == false then
			...
			if TukuiCF["others"].enablebag == true and StuffingFrameBags:IsShown() then
				...
				if not HasPetUI() then
					self:SetPoint("BOTTOMRIGHT", TukuiActionBarBackgroundRight, "TOPRIGHT", -1, TukuiDB.Scale(18))		
				else
					self:SetPoint("BOTTOMRIGHT", TukuiPetActionBarBackground, "TOPRIGHT", -1, TukuiDB.Scale(18))	
				end
			end
		end
	end
