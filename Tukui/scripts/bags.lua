...
if not TukuiCF["others"].enablebag == true then 
	local MoveBags = function()
		...
		ContainerFrame1:SetPoint("BOTTOMRIGHT", ChatRBackground, "BOTTOMRIGHT", 0, TukuiDB.Scale(5))
...
local ST_QUIVER = 4
local bag_bars = 1
local hide_soulbag = TukuiCF["others"].soulbag
...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		...
		if TukuiCF["actionbar"].rightbars > 0 and TukuiCF["actionbar"].rightbars_vh then
         f:SetPoint("BOTTOMRIGHT", TukuiActionBarBackgroundRight, "BOTTOMLEFT", TukuiDB.Scale(-3), 0)
      elseif TukuiCF["actionbar"].rightbars > 0 and not TukuiCF["actionbar"].rightbars_vh then
         f:SetPoint("BOTTOMRIGHT", TukuiActionBarBackgroundRight, "TOPRIGHT")
      else
...
function Stuffing:Layout(lb)
	...
	if lb then
		bs = bags_BANK
		cols = (floor(TukuiDB.infowidth/370 * 10))
		f = self.bankFrame
	else
		bs = bags_BACKPACK
		cols = (floor(TukuiDB.infowidth/370 * 10))
	...
	if bag_bars == 1 then
		...
		local bsize = TukuiDB.Scale(29)
		if lb then bsize = TukuiDB.Scale(29) end

		local w = 2 * 12
      w = w + ((#bs - 1) * bsize)
	   w = w + (12 * (#bs - 4))
              
--		if lb thenw = w + ((#bs -2) * bsize)
--		   w = w + (12 * (#bs - 3))
--		else		
--         w = w + ((#bs - 1) * bsize)
--		   w = w + (12 * (#bs - 4))
--		end
	...
	for _, v in ipairs(bs) do
		if (not lb and v <= 3 ) or (lb and v ~= -1) then
			local bsize = TukuiDB.Scale(29)
			if lb then TukuiDB.Scale(29) end
			...
			b.frame:SetWidth(TukuiDB.Scale(bsize))
			b.frame:SetHeight(TukuiDB.Scale(bsize))
	...
	f:SetWidth(TukuiDB.Scale(TukuiDB.infowidth))
...
function Stuffing:PLAYER_ENTERING_WORLD()
	...
	ContainerFrame1:HookScript("OnShow", function(self)
		ContainerFrame1:ClearAllPoints()
		ContainerFrame1:SetPoint("TOPRIGHT", StuffingFrameBags, "TOPLEFT", TukuiDB.Scale(2), TukuiDB.Scale(40))
	end)
