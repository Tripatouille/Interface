...
Mod_AddonSkins:SetScript("OnEvent",function(self, event, addon)
	...
	if TukuiCF["general"].embedright == "Skada" and IsAddOnLoaded("Skada") then
		...
		SkadaBarWindowSkada:SetPoint("TOPRIGHT", RDummyFrame, "TOPRIGHT", -2, -2)
		local function AdjustSkadaFrameLevels()
			...
			if SkadaBG then
				...
				SkadaBG:SetAllPoints(RDummyFrame)
			end
		end
	...
	if TukuiCF["general"].recount and IsAddOnLoaded("Recount") then
		Recount_MainWindow:ClearAllPoints()
		Recount_MainWindow:SetPoint("TOPLEFT", RDummyFrame ,"TOPLEFT", 0, 7)
		Recount_MainWindow:SetPoint("BOTTOMRIGHT", RDummyFrame ,"BOTTOMRIGHT", 0, 0)
		Recount.db.profile.MainWindowWidth = (TukuiCF["chat"].chatwidth - 4)
	end
	if TukuiCF["general"].omen and IsAddOnLoaded("Omen") then
		OmenTitle:SetPoint("TOPLEFT", OmenDummyFrame, "TOPLEFT")
		OmenTitle:SetPoint("TOPRIGHT", OmenDummyFrame, "TOPRIGHT")
		OmenBarList:SetPoint("BOTTOMLEFT",OmenDummyFrame, "BOTTOMLEFT")
		OmenBarList:SetPoint("BOTTOMRIGHT",OmenDummyFrame, "BOTTOMRIGHT")
	end
end)