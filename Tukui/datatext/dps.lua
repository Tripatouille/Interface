...
if TukuiCF["datatext"].dps_text and TukuiCF["datatext"].dps_text > 0 then
	...
	dText  = TukuiDataLeft:CreateFontString(nil, "OVERLAY")
	dText:SetFont(TukuiCF.theme.DATA_Font, TukuiCF.theme.DATA_FSize, TukuiCF.theme.DATA_FFlag)
	...
	function getDPS()
		if (dmg_total == 0) then
			return ("0.0 " .. cStart .. tukuilocal.datatext_dps)
		else
			return string.format("%.1f " .. cStart .. tukuilocal.datatext_dps .. cEnd, (dmg_total or 0) / (cmbt_time or 1))
		end
	end
